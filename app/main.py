...
if __name__ == '__main__':
    project_folder = os.path.expanduser('~/Desktop/Projects/bitlinks_consider_clicks/app')
    load_dotenv(os.path.join(project_folder, '.env'))
    token = {'Authorization': os.getenv('BITLY_API_KEY')}
    parser = argparse.ArgumentParser(description='Сокращает ссылки и показывает количество кликов по сокращенной ссылке')
